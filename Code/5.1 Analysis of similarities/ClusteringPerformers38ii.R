...
## --------------------------------------------------------------------------------------------------
tbeats <- read.csv("C:/Users/samte/OneDrive/Desktop/TFM Clustering of musical patterns/curatedData/tbeats38ii.csv")
tintensity <- read.csv("C:/Users/samte/OneDrive/Desktop/TFM Clustering of musical patterns/curatedData/tintensity38ii.csv")
combined <-cbind(tbeats[,1:1320],tintensity)
...
## --------------------------------------------------------------------------------------------------
pdf(file="C:/Users/samte/OneDrive/Desktop/TFM Clustering of musical patterns/plots/Heatmap38ii.pdf",
    width = 8, height = 8)

rownames(combined)<-combined$performers
dist<-dist(scale(combined[,1:2640]))
...
pdf(file="C:/Users/samte/OneDrive/Desktop/TFM Clustering of musical patterns/plots/Dendogram38ii.pdf",
    width = 8, height = 8)
...
## --------------------------------------------------------------------------------------------------
pca <- prcomp(combined[,1:2640], center = T, scale. = T)
summary(pca)
...
#PCA + k-means
## --------------------------------------------------------------------------------------------------
pcab<-prcomp(tbeats[,1:1320], center = T, scale. = T)
summary(pcab)
...
pcai<-prcomp(tintensity[,1:1320], center = T, scale. = T)
summary(pcai)
...
for (k in 3:8){
  for (i in 1:100){
    a <- kmeans(scale(combined[,1:2640]), centers = k,iter.max = 100000)
    m[i+(k-3)*100,1] <- a$tot.withinss
    n[i+(k-3)*100,] <- a$cluster
    m[i+(k-3)*100,2] <- mean(silhouette(as.vector(n[i+(k-3)*100,]),dist(scale(combined[,1:2640])))[,3])
    m[i+(k-3)*100,3] <- k
...
for (k in 3:8){
  for (i in 1:100){
    a <- kmeans(scale(tbeats[,1:1320]), centers = k,iter.max = 100000)
    m[i+(k-3)*100,1] <- a$tot.withinss
    n[i+(k-3)*100,] <- a$cluster
    m[i+(k-3)*100,2] <- mean(silhouette(as.vector(n[i+(k-3)*100,]),dist(scale(tbeats[,1:1320])))[,3])
    m[i+(k-3)*100,3] <- k
...
for (k in 3:8){
  for (i in 1:100){
    a <- kmeans(scale(tintensity[,1:1320]), centers = k,iter.max = 100000)
    m[i+(k-3)*100,1] <- a$tot.withinss
    n[i+(k-3)*100,] <- a$cluster
    m[i+(k-3)*100,2] <- mean(silhouette(as.vector(n[i+(k-3)*100,]),dist(scale(tintensity[,1:1320])))[,3])
    m[i+(k-3)*100,3] <- k
...
KL <- 9999
for (k in 1:1000){
  tsne_obj <- Rtsne(scale(combined[,1:2640]), dims = 2, perplexity = 4, max_iter = 2500, pca = F)
...
KL <- 9999
for (k in 1:500){
  tsne_obj <- Rtsne(scale(tbeats[,1:1320]), dims = 2, perplexity = 4, max_iter = 2500, pca = F)
  tsne2 <- as.data.frame(tsne_obj$Y)
  newKL<-tsne_obj$itercosts[50]
...
KL <- 9999
for (k in 1:500){
  tsne_obj <- Rtsne(scale(tintensity[,1:1320]), dims = 2, perplexity = 4, max_iter = 2500, pca = F)
  tsne2 <- as.data.frame(tsne_obj$Y)
  newKL<-tsne_obj$itercosts[50]
...
## --------------------------------------------------------------------------------------------------
pdf(file="C:/Users/samte/OneDrive/Desktop/TFM Clustering of musical patterns/plots/PCAplotbi38ii.pdf",
    width = 16, height = 4.5)
...
pdf(file="C:/Users/samte/OneDrive/Desktop/TFM Clustering of musical patterns/plots/PCAplotb38ii.pdf",
    width = 16, height = 4.5)
...
pdf(file="C:/Users/samte/OneDrive/Desktop/TFM Clustering of musical patterns/plots/PCAploti38ii.pdf",
    width = 16, height = 4.5)
